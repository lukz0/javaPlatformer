...
public class Renderer implements Runnable {
        ...
        while (this.shouldRun) {
            if (!this.taskQueue.isEmpty()) {
                ...
                do {
                    ...
                    tasksInQueue.forEach((tsk) -> tsk.doTask(this));
                } while (!this.taskQueue.isEmpty());
            }
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // clear the framebuffer

            long currentTimeStamp = System.nanoTime();
            this.drawnElements.forEach((id, drawable) -> drawable.draw(currentTimeStamp));
    ...
    class PosUpdateableGroup implements PosUpdateable {
        ...
        public void delete() {
            states.forEach((stateID, state) -> state.delete());
        }
    ...
    private class GetNewPosUpdateableGroupTask implements Task {
        ...
        public void doTask(Renderer r) {
            ...
            states.forEach((key, value) -> {
                try {
                    nonasyncStates.put(key, (PosUpdateable) value.get());
                } catch(Exception e) {
                    System.err.println("Drawable with ID ".concat(Integer.toString(key).concat(" doesn't implement PosUpdateable")));
                }
            });
            try {
